...
class InstalasiGawatDaruratController extends Controller
{
    ...
    {
        $data = Service::where('type', 'igd')->first();
        return view('admin.gawat-darurat.index', compact('data'));
    }
    /**
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            // Mengambil data Service yang type-nya 'igd'
            ...
            if ($request->hasFile('img')) {
                ...
                $filename = 'igd_' . now()->format('YmdHis') . '_' . Str::random(5) . '.' . $image->getClientOriginalExtension();
            ...
            return redirect()->route('admin.gawat-darurat.index')->with('success', 'Service berhasil diperbarui.');
        } catch (\Exception $e) {
            DB::rollBack();
        ...
        if ($request->ajax()) {
            ...
            return datatables()->of($data)
                ...
                ->addColumn('action', function($data) {
                    return view('admin.gawat-darurat.sub-service.datatables.action', compact('data'));
                })
                ->rawColumns(['action'])
                ->make(true);
        }
        return view('admin.gawat-darurat.sub-service.index', compact('id'));
    }

    public function createSubService($id)
    {
        return view('admin.gawat-darurat.sub-service.create', compact('id'));
    }
    ...
    try {
        ...
        return redirect()->route('admin.gawat-darurat.sub-service.index', $id)->with('success', 'Sub Service berhasil ditambahkan.');
    } catch (\Exception $e) {
    ...
    public function editSubService($slug)
    {
        ...
        return view('admin.gawat-darurat.sub-service.edit', compact('subService'));
    }
    ...
    public function updateSubService(Request $request, $slug)
    {
        ...
        try {
            ...
            return redirect()->route('admin.gawat-darurat.sub-service.index', $rawatJalan->service_id)->with('success', 'Sub Service berhasil diperbarui.');
    ...
    {
        if ($request->ajax()) {
            $data = Gallery::where('type', 'igd')->orderBy('created_at', 'asc')->get();

            return DataTables::of($data)
             ->addIndexColumn()
             ->addColumn('action', 'admin.gawat-darurat.gallery.datatables.action')
         ...
         $id = Service::where('type', 'igd')->select('id')->first();
         return view('admin.gawat-darurat.gallery.index', compact('id'));
    }

    public function galleryCreate()
    {
        return view('admin.gawat-darurat.gallery.create');
    }
    ...
    public function galleryStore(Request $request)
    {
        ...
        try {
            if ($request->hasFile('img')) {
                ...
                $maxSort = Gallery::where('type', 'igd')->max('sort');
                ...
                return redirect()->route('admin.gawat-darurat.gallery.index')->with('success', 'Gambar berhasil diunggah');
            }
    ...
    public function galleryEdit($id)
    {
        ...
        return view('admin.gawat-darurat.gallery.edit', compact('data'));
    }
    ...
    public function galleryUpdate(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('admin.gawat-darurat.gallery.index')->with('success', 'Gambar berhasil diperbarui');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Terjadi kesalahan: ' . $e->getMessage());
        }
