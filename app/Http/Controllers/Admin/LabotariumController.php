...
class LabotariumController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            // Mengambil data Service yang type-nya 'lab'
            ...
            if ($request->hasFile('img')) {
                ...
                $filename = 'lab_' . now()->format('YmdHis') . '_' . Str::random(5) . '.' . $image->getClientOriginalExtension();
    ...
    {
        if ($request->ajax()) {
            $data = Gallery::where('type', 'lab')->orderBy('created_at', 'asc')->get();
    ...
    public function galleryStore(Request $request)
    {
        ...
        try {
            if ($request->hasFile('img')) {
                ...
                $maxSort = Gallery::where('type', 'lab')->max('sort');
