...
class RehabMedikController extends Controller
{
    ...
    {
        $data = Service::where('type', 'rehab_medik')->first();
        return view('admin.rehab-medik.index', compact('data'));
    }
    /**
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            // Mengambil data Service yang type-nya 'rehab-medik'
            ...
            if ($request->hasFile('img')) {
                ...
                $filename = 'rehab_medik_' . now()->format('YmdHis') . '_' . Str::random(5) . '.' . $image->getClientOriginalExtension();
            ...
            return redirect()->route('admin.rehab-medik.index')->with('success', 'Service berhasil diperbarui.');
        } catch (\Exception $e) {
            DB::rollBack();
        ...
        if ($request->ajax()) {
            ...
            return datatables()->of($data)
                ...
                ->addColumn('action', function($data) {
                    return view('admin.rehab-medik.sub-service.datatables.action', compact('data'));
                })
                ->rawColumns(['action'])
                ->make(true);
        }
        return view('admin.rehab-medik.sub-service.index', compact('id'));
    }

    public function createSubService($id)
    {
        return view('admin.rehab-medik.sub-service.create', compact('id'));
    }
    ...
    try {
        ...
        return redirect()->route('admin.rehab-medik.sub-service.index', $id)->with('success', 'Sub Service berhasil ditambahkan.');
    } catch (\Exception $e) {
    ...
    public function editSubService($slug)
    {
        ...
        return view('admin.rehab-medik.sub-service.edit', compact('subService'));
    }
    ...
    public function updateSubService(Request $request, $slug)
    {
        ...
        try {
            ...
            return redirect()->route('admin.rehab-medik.sub-service.index', $rawatJalan->service_id)->with('success', 'Sub Service berhasil diperbarui.');
    ...
    {
        if ($request->ajax()) {
            $data = Gallery::where('type', 'rehab_medik')->orderBy('created_at', 'asc')->get();

            return DataTables::of($data)
             ->addIndexColumn()
             ->addColumn('action', 'admin.rehab-medik.gallery.datatables.action')
         ...
         $id = Service::where('type', 'rehab_medik')->select('id')->first();
         return view('admin.rehab-medik.gallery.index', compact('id'));
    }

    public function galleryCreate()
    {
        return view('admin.rehab-medik.gallery.create');
    }
    ...
    public function galleryStore(Request $request)
    {
        ...
        try {
            if ($request->hasFile('img')) {
                ...
                $maxSort = Gallery::where('type', 'rehab_medik')->max('sort');
                $sort = $maxSort ? $maxSort + 1 : 1;

                $gallery = new Gallery();
                $gallery->img = $path;
                $gallery->type = 'rehab_medik';
                $gallery->sort = $sort;
                $gallery->save();

                DB::commit();

                return redirect()->route('admin.rehab-medik.gallery.index')->with('success', 'Gambar berhasil diunggah');
            }
    ...
    public function galleryEdit($id)
    {
        ...
        return view('admin.rehab-medik.gallery.edit', compact('data'));
    }
    ...
    public function galleryUpdate(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('admin.rehab-medik.gallery.index')->with('success', 'Gambar berhasil diperbarui');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Terjadi kesalahan: ' . $e->getMessage());
        }
