...
class InstalasiRawatInapController extends Controller
{
    ...
    {
        $data = Service::where('type', 'iri')->first();
        return view('admin.rawat-inap.index', compact('data'));
    }
    /**
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            // Mengambil data Service yang type-nya 'iri'
            ...
            if ($request->hasFile('img')) {
                ...
                $filename = 'iri_' . now()->format('YmdHis') . '_' . Str::random(5) . '.' . $image->getClientOriginalExtension();
            ...
            return redirect()->route('admin.rawat-inap.index')->with('success', 'Service berhasil diperbarui.');
        } catch (\Exception $e) {
            DB::rollBack();
        ...
        if ($request->ajax()) {
            ...
            return datatables()->of($data)
                ...
                ->addColumn('action', function($data) {
                    return view('admin.rawat-inap.sub-service.datatables.action', compact('data'));
                })
                ->rawColumns(['action'])
                ->make(true);
        }
        return view('admin.rawat-inap.sub-service.index', compact('id'));
    }

    public function createSubService($id)
    {
        return view('admin.rawat-inap.sub-service.create', compact('id'));
    }
    ...
    try {
        ...
        return redirect()->route('admin.rawat-inap.sub-service.index', $id)->with('success', 'Sub Service berhasil ditambahkan.');
    } catch (\Exception $e) {
    ...
    public function editSubService($slug)
    {
        ...
        return view('admin.rawat-inap.sub-service.edit', compact('subService'));
    }
    ...
    public function updateSubService(Request $request, $slug)
    {
        ...
        try {
            ...
            return redirect()->route('admin.rawat-inap.sub-service.index', $rawatJalan->service_id)->with('success', 'Sub Service berhasil diperbarui.');
    ...
    {
        if ($request->ajax()) {
            $data = Gallery::where('type', 'iri')->orderBy('created_at', 'asc')->get();

            return DataTables::of($data)
             ->addIndexColumn()
             ->addColumn('action', 'admin.rawat-inap.gallery.datatables.action')
         ...
         $id = Service::where('type', 'iri')->select('id')->first();
         return view('admin.rawat-inap.gallery.index', compact('id'));
    }

    public function galleryCreate()
    {
        return view('admin.rawat-inap.gallery.create');
    }
    ...
    public function galleryStore(Request $request)
    {
        ...
        try {
            if ($request->hasFile('img')) {
                ...
                $maxSort = Gallery::where('type', 'iri')->max('sort');
                ...
                return redirect()->route('admin.rawat-inap.gallery.index')->with('success', 'Gambar berhasil diunggah');
            }
    ...
    public function galleryEdit($id)
    {
        ...
        return view('admin.rawat-inap.gallery.edit', compact('data'));
    }
    ...
    public function galleryUpdate(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('admin.rawat-inap.gallery.index')->with('success', 'Gambar berhasil diperbarui');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Terjadi kesalahan: ' . $e->getMessage());
        }
