...
class AmbulanceController extends Controller
{
    ...
    {
        $data = Service::where('type', 'ambulance')->first();
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            // Mengambil data Service yang type-nya 'ambulance'
            ...
            if ($request->hasFile('img')) {
                ...
                $filename = 'ambulance_' . now()->format('YmdHis') . '_' . Str::random(5) . '.' . $image->getClientOriginalExtension();
            ...
            return redirect()->route('admin.ambulance.index')->with('success', 'Service berhasil diperbarui.');
        } catch (\Exception $e) {
            DB::rollBack();
        ...
        if ($request->ajax()) {
            ...
            return datatables()->of($data)
                ...
                ->addColumn('action', function($data) {
                    return view('admin.ambulance.sub-service.datatables.action', compact('data'));
                })
                ->rawColumns(['action'])
                ->make(true);
        }
        return view('admin.ambulance.sub-service.index', compact('id'));
    }

    public function createSubService($id)
    {
        return view('admin.ambulance.sub-service.create', compact('id'));
    }
    ...
    try {
        ...
        return redirect()->route('admin.ambulance.sub-service.index', $id)->with('success', 'Sub Service berhasil ditambahkan.');
    } catch (\Exception $e) {
    ...
    public function editSubService($slug)
    {
        ...
        return view('admin.ambulance.sub-service.edit', compact('subService'));
    }
    ...
    public function updateSubService(Request $request, $slug)
    {
        ...
        try {
            ...
            return redirect()->route('admin.ambulance.sub-service.index', $rawatJalan->service_id)->with('success', 'Sub Service berhasil diperbarui.');
    ...
    {
        if ($request->ajax()) {
            $data = Gallery::where('type', 'ambulance')->orderBy('created_at', 'asc')->get();

            return DataTables::of($data)
             ->addIndexColumn()
             ->addColumn('action', 'admin.ambulance.gallery.datatables.action')
         ...
         $id = Service::where('type', 'ambulance')->select('id')->first();
         return view('admin.ambulance.gallery.index', compact('id'));
    }

    public function galleryCreate()
    {
        return view('admin.ambulance.gallery.create');
    }
    ...
    public function galleryStore(Request $request)
    {
        ...
        try {
            if ($request->hasFile('img')) {
                ...
                $maxSort = Gallery::where('type', 'ambulance')->max('sort');
                $sort = $maxSort ? $maxSort + 1 : 1;

                $gallery = new Gallery();
                $gallery->img = $path;
                $gallery->type = 'ambulance';
                $gallery->sort = $sort;
                $gallery->save();

                DB::commit();

                return redirect()->route('admin.ambulance.gallery.index')->with('success', 'Gambar berhasil diunggah');
            }
    ...
    public function galleryEdit($id)
    {
        ...
        return view('admin.ambulance.gallery.edit', compact('data'));
    }
    ...
    public function galleryUpdate(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('admin.ambulance.gallery.index')->with('success', 'Gambar berhasil diperbarui');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Terjadi kesalahan: ' . $e->getMessage());
        }
