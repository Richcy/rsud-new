...
class LayananUnggulanController extends Controller
{
    ...
    {
        $data = Service::where('type', 'unggulan')->first();
        return view('admin.layanan-unggulan.index', compact('data'));
    }
    /**
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            // Mengambil data Service yang type-nya 'unggulan'
            ...
            if ($request->hasFile('img')) {
                ...
                $filename = 'unggulan_' . now()->format('YmdHis') . '_' . Str::random(5) . '.' . $image->getClientOriginalExtension();
        ...
        if ($request->ajax()) {
            ...
            return datatables()->of($data)
                ...
                ->addColumn('action', function($data) {
                    return view('admin.layanan-unggulan.sub-service.datatables.action', compact('data'));
                })
                ->rawColumns(['action'])
                ->make(true);
        }
        return view('admin.layanan-unggulan.sub-service.index', compact('id'));
    }

    public function createSubService($id)
    {
        return view('admin.layanan-unggulan.sub-service.create', compact('id'));
    }
    ...
    try {
        ...
        return redirect()->route('admin.layanan-unggulan-sub-service.index', $id)->with('success', 'SubService berhasil ditambahkan.');
    } catch (\Exception $e) {
    ...
    public function editSubService($slug)
    {
        ...
        return view('admin.layanan-unggulan.sub-service.edit', compact('subService'));
    }
    ...
    public function updateSubService(Request $request, $slug)
    {
        ...
        try {
            ...
            return redirect()->route('admin.layanan-unggulan-sub-service.index', $subService->service_id)->with('success', 'Sub Service berhasil diperbarui.');
    ...
    public function destroySubService($slug)
    {
        ...
        try {
            $subService = SubService::where('slug', $slug);
    ...
    {
        if ($request->ajax()) {
            $data = Gallery::where('type', 'unggulan')->orderBy('created_at', 'asc')->get();

            return DataTables::of($data)
             ->addIndexColumn()
             ->addColumn('action', 'admin.layanan-unggulan.gallery.datatables.action')
         ...
         $id = Service::where('type', 'unggulan')->select('id')->first();
         return view('admin.layanan-unggulan.gallery.index', compact('id'));
    }

    public function galleryCreate()
    {
        return view('admin.layanan-unggulan.gallery.create');
    }
    ...
    public function galleryStore(Request $request)
    {
        ...
        try {
            if ($request->hasFile('img')) {
                ...
                $maxSort = Gallery::where('type', 'unggulan')->max('sort');
                $sort = $maxSort ? $maxSort + 1 : 1;

                $gallery = new Gallery();
                $gallery->img = $path;
                $gallery->type = 'unggulan';
                $gallery->sort = $sort;
                $gallery->save();

                DB::commit();

                return redirect()->route('admin.layanan-unggulan-gallery.index')->with('success', 'Gambar berhasil diunggah');
            }
    ...
    public function galleryEdit($id)
    {
        ...
        return view('admin.layanan-unggulan.gallery.edit', compact('data'));
    }
    ...
    public function galleryUpdate(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('admin.layanan-unggulan-gallery.index')->with('success', 'Gambar berhasil diperbarui');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Terjadi kesalahan: ' . $e->getMessage());
        }
